...
class plgJeaSocial extends JPlugin
{
    ...
    function showSocialBar(&$row) {
        ...
        if(!is_null($this->_params)) {
            ...
            $like = '';  $retweet = '';  $buzz = '';  $digg = '';
            // ensure that we are viewing a property
        	if(JRequest::getVar('view','')=='properties' && isset($row->id)) {
        	    // build the property url
                if(isset($row->id)) $url = "http://".$_SERVER['HTTP_HOST'].JRoute::_('index.php?option=com_jea&view=properties&id='.$row->slug);
            }
            else return;
            $url2 = urlencode($url);
            // facebook             
